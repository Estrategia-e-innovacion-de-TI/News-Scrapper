"""Capability: clustering — ClusterEngine for TF-IDF based thematic grouping.
...
def _compute_trend_timeline(
    ...
    # Accumulate: (month, topic) -> list of scores
...
def _compute_hype_indicators(
    ...
    """
    # Group timeline entries by topic
    ...
    # Get sorted months across all topics
    ...
    for cluster in clusters:
        ...
        # Compute momentum from last 3 months
...
def _calculate_momentum(
    ...
    # Rate of change: compare last period to first period
    first = counts[0]
    last = counts[-1]
    ...
    # Normalise to [0, 1]: -1..+inf → 0..1 via sigmoid-like mapping
...
class ClusterEngine:
    ...
    def cluster(self, items: list[dict[str, Any]]) -> ClusteringOutput:
        ...
        # <6 items → single "all_items" cluster, skip TF-IDF
        ...
        try:
            ...
            logger.error("Clustering failed — returning all items as single cluster", exc_info=True)
            return self._single_cluster(items)
    ...
    def _sklearn_cluster(self, items: list[dict[str, Any]]) -> ClusteringOutput:
        ...
        # Combine English + Spanish stop words
        ...
        # 4. Merge small clusters (<min_cluster_size) into nearest or "unclustered"
        ...
        for cluster_label, indices in sorted(cluster_groups.items()):
            ...
            # Compute centroid and extract top 4 keywords
            ...
            keywords = [str(feature_names[i]) for i in top_indices if centroid[i] > 0]
            # Pad to 4 if fewer non-zero terms
            keywords = keywords[:4]

            label_str = " ".join(keywords) if keywords else f"cluster_{cluster_label}"
            cid = f"cluster_{cluster_label}" if cluster_label >= 0 else "unclustered"
        ...
        # 6. Compute trend_timeline and hype_indicators
    ...
    def _merge_small_clusters(
        ...
        """Merge clusters with <min_cluster_size items into nearest cluster or unclustered."""
        import numpy as np
        from sklearn.metrics.pairwise import cosine_similarity
        ...
        # If no large clusters exist, keep everything as-is (unclustered)
        if not large:
...
def _cluster_items(items: list[dict[str, Any]], **kwargs: Any) -> ClusteringOutput:
    """Thin wrapper for capability registry."""
    engine = ClusterEngine(**kwargs)
    return engine.cluster(items)


_cap_clustering = CapabilityDef(
    name="cluster_engine",
    purpose="Group items by thematic similarity using TF-IDF + cosine similarity, compute trends and hype indicators",
    inputs_schema={
        "items": "list[dict] — each with title, excerpt/text, url, score, published_at",
    },
    outputs_schema={
        "clusters": "list[ClusterResult]",
        "trend_timeline": "list[TrendEntry]",
    ...
    callable=_cluster_items,
    tags=["clustering", "vigilancia", "historical"],
)
register_capability(_cap_clustering)
