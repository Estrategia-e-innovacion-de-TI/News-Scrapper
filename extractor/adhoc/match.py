"""Metadata-only keyword matching for ad-hoc queries (Opción 1).

Matches terms against title/summary/snippet WITHOUT fetching full articles.
This is the fast, cheap path for ARAS and Riesgos ad-hoc runs.
"""
...
def metadata_match(
    ...
    # Extract URL slug for matching (path portion, hyphens → spaces)
    norm_url = ""
    if url:
        from urllib.parse import urlparse
        path = urlparse(url).path
        norm_url = normalize_text(path.replace("-", " ").replace("_", " ").replace("/", " "))
