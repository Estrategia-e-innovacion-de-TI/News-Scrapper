...
Ported from news_radar_mvp/extractor/search/providers/epo_patents.py.
...
def _parse_search_xml(xml_text: str) -> list[dict]:
    ...
    try:
        ...
        for ref in root.iter(f"{{{_NS_EXCH}}}publication-reference"):
            doc_id = ref.find(f"{{{_NS_EXCH}}}document-id")
            if doc_id is None:
                continue
            country_el = doc_id.find(f"{{{_NS_EXCH}}}country")
            docnum_el = doc_id.find(f"{{{_NS_EXCH}}}doc-number")
            kind_el = doc_id.find(f"{{{_NS_EXCH}}}kind")
...
def _parse_biblio_xml(xml_text: str) -> dict[str, dict]:
    ...
    try:
        root = ET.fromstring(xml_text)
        for doc in root.iter(f"{{{_NS_EXCH}}}exchange-document"):
            ...
            title = ""
            for t in doc.iter(f"{{{_NS_EXCH}}}invention-title"):
                lang = t.get("lang", "")
                text = t.text or ""
            ...
            abstract = ""
            for ab in doc.iter(f"{{{_NS_EXCH}}}abstract"):
                lang = ab.get("lang", "")
                parts = []
                for p in ab.iter(f"{{{_NS_EXCH}}}p"):
                    if p.text:
            ...
            # Applicants
            applicants: list[str] = []
            for app in doc.iter(f"{{{_NS_EXCH}}}applicant"):
                name_el = app.find(f"{{{_NS_EXCH}}}name")
...
async def search_epo_patents(
    ...
    # Search in weekly chunks to avoid EPO 413
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False, timeout=timeout) as client:
                ...
                if resp.status_code == 404:
                    ...
                    logger.warning(
                        "EPO 413 for '%s' chunk %s-%s, narrowing", term, d_from, d_to,
                    )
                else:
                    resp.raise_for_status()
    ...
    # Fetch bibliographic data in batches
    ...
    for ref in doc_refs:
        ...
        display_title = (
            f"[{ref['country']} {ref['doc_number']}] {title}"
            if title
            else f"[{key}] Patent"
        )
        snippet = abstract
